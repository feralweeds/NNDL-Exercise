...
def pretty_print_poem(poem):  # 令打印的结果更工整
    # 去掉起始和结束标记
    poem_clean = poem.replace(start_token, '').replace(end_token, '')
    poem_sentences = poem_clean.split('。')
...
for w in begin_words:
    print(f"\n【{w}】开头的诗：")
    pretty_print_poem(gen_poem(w, word_int_map, vocabularies, rnn_model))